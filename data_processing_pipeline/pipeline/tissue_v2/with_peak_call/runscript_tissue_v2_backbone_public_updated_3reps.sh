...
BAM_NAME1="wgEncodeOpenChromDnaseHepg2AlnRep1.bam"
BAM_NAME2="wgEncodeOpenChromDnaseHepg2AlnRep2.bam"
BAM_NAME3="wgEncodeOpenChromDnaseHepg2AlnRep3.bam"
#set names to assign
BAM_MERGED="wgEncodeOpenChromDnaseHepg2Alnmerged.bam"
...
BAM_FILE3=${OUTPUT_DIR}/${BAM_NAME3}
	...
	mouse)
		...
		case "${BUILD}" in
			...
			mm9)	
				...
				BIGWIG_CHRSIZES='/t1-data1/user/config/bigwig/mm9_sizes.txt'
				#ploidy regions to filter
...
#rep3 call
macs -t ${BAM_FILE3} --name Rep3 --format="BAM"
TEMP_REP_PEAK2=${OUTPUT_DIR}/Rep3_peaks.bed
if [ $? -ne 0 ]
then
  echo "Error in MACS peak calling.. not enough reads?"
  exit -1
...
#filter for chrM
grep -v chrM ${TEMP_REP_PEAK3} >temp
mv -f temp ${TEMP_REP_PEAK3}

#With All: merge and sort
cat ${TEMP_REP_PEAK1} ${TEMP_REP_PEAK2} ${TEMP_REP_PEAK3} >merged
...
bedtools intersect -u -a merged.sorted.collapsed -b ${TEMP_REP_PEAK1} >temp2
bedtools intersect -u -a temp2 -b ${TEMP_REP_PEAK2} >temp
bedtools intersect -u -a temp -b ${TEMP_REP_PEAK3} >${REGIONS_FILE}
...
#remove temp files
rm -f merged merged.sorted.collapsed temp merged.sorted temp2
...
samtools merge ${BAM_FILE} ${BAM_FILE1} ${BAM_FILE2} ${BAM_FILE3}
