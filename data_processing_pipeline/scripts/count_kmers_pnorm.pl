...
#read in propensity file and store norm factor hash
my %norm_hash;
...
open(IN, $peak_in) or die "Can't open peaks input file $peak_in $!";
    ...
    while (<IN>) {
       ...
       my $chromosome = $col[0];
       	...
       	next if $chromosome !~ /chr\w+/;

#get start and stop position coordinates accoring to file format and appropriate indexing (0 or 1 start)
        if( ($peak_in =~ /\.bed$/) || ($peak_in =~ /\.narrowPeak$/) ) {	#if bed format
            
            $peakstart = $col[1] - $extend_kmersearch - $extend_peak - 3 + 1;       #extend for kmer search if wanted +1 for bed 0-based
            $peakstop = $col[2] + $extend_kmersearch + $extend_peak + 2;

            
        }elsif( $peak_in =~ /\.gff$/ ) {
            
            $peakstart = $col[3] - $extend_kmersearch - $extend_peak - 3;       #extend for kmer search if wanted
            $peakstop = $col[4] + $extend_kmersearch + $extend_peak + 2;	#extend with 3 bp to alwas complete the 6mer for bias norm
	...
	my $startkmerssave = $extend_peak + 3 - 1; #where to start saving the kmers (actual peak is extended by 150 bp +- 3bp for completing the 6mers for pnorm so 150 + 3 and - 1 for perl 0 based indexing

        my $count=0;

        #go through sequence and count up kmers of length found in it
        while (length($subseq) >= ($length + ($extend_peak * 2) + 5) ) {		#until length(6) + 153 flanking / +5 to always complete the 6mer for 6mer propensities
            
		$start=$peakstart+$count+3; #get borders for searching the footprint (kmer length +- $extend_peak up and down stream) / +3 for 6mer completion

		#outdated###$stop=$start+$length+(2*$extend_peak)-1;    #-1 norms for perl array index
	    ...
	    if(exists $kmers{$kmer}){   #recently added / only proper kmers without Ns considered
		$kmers{$kmer}++;
