...
tab <- cbind(table1[,c(2,1,3,4)], table2[,c(1,3,4)])

#add difference in fsr columns
tab[,(dim(tab)[2]+1)] <- tab[,3] - tab[,6]
tab[,(dim(tab)[2]+1)] <- tab[,4] - tab[,7]
...
names(tab) <- c("id", "kmer1", "SFR_plus1", "SFR_minus1", "kmer2", "SFR_plus2", "SFR_minus2", "DMG_plus", "DMG_minus", "var_flag")

#order table by difference 
tab <- tab[order(tab[,8],decreasing=TRUE),]
