...
#make plots in lists
profile.naked.list <- plot.profile.background.cutting(kmer, profile.plus=profile.plus, profile.minus=profile.minus, count.plus=count.plus, count.minus=count.minus, smooth.flag=smooth.flag)
...
#plus
png.file=paste0(plot.dir,'/naked_cut_plot_',kmer,'_plus.png')
png(png.file, width=900, height=300)

		print( profile.naked.list$pp )	#real

suppress.output <- dev.off()

#minus
png.file=paste0(plot.dir,'/naked_cut_plot_',kmer,'_minus.png')
png(png.file, width=900, height=300)

		print( profile.naked.list$pm )	#real
